...
export default class AvaTaxClient {
   ...
   * @param number customHttpAgent      Specify the http agent which will be used to make http requests to the Avatax APIs.
   */
  ...
  restCall({ url, verb, payload, clientId = '', mapHeader = new Map() }) {
    ...
    return withTimeout(this.timeout, fetch(url, options)).then(res => {
      ...
      if (contentType && contentType.includes('application/json')) {
        if ((contentLength == 0 && parseInt(res.status/100) == 2) || res.status == 204 ){
          return null;
        }
      }
   ...
   * @return object
   */
  createFundingRequest({ id, businessUnit, subscriptionType, model } = {}) {
    var path = this.buildUrl({
   ...
   * @return object[]
   */
  listACHEntryDetailsForCompany({ id, periodyear, periodmonth } = {}) {
    var path = this.buildUrl({
   ...
   * @return object
   */
  queryTaxAuthorityJurisdictionRates({ taxAuthorityId, effectiveDate, endDate, filter, include, top, skip, orderBy } = {}) {
    var path = this.buildUrl({
   ...
   * @return FetchResult
   */
  listJurisdictionsByRateTypeTaxTypeMapping({ country, taxTypeId, taxSubTypeId, rateTypeId, region, filter, top, skip, orderBy } = {}) {
    var path = this.buildUrl({
   ...
   * @return string[]
   */
  listJurisdictionTypesByRateTypeTaxTypeMapping({ country, taxTypeId, taxSubTypeId, rateTypeId } = {}) {
    var path = this.buildUrl({
   ...
   * @return object
   */
  activateFundingRequest({ id, businessUnit, subscriptionType } = {}) {
    var path = this.buildUrl({
   ...
   * @return object
   */
  fundingRequestStatus({ id, businessUnit, subscriptionType } = {}) {
    var path = this.buildUrl({
   ...
   *
   * 
     * @param int companyId The ID of the company creating this request.
     * @param object model The request you wish to create.
   * @return object
   */
  createTaxCodeClassificationRequest({ companyId, model } = {}) {
    var path = this.buildUrl({
   ...
   * @return object[]
   */
  deleteCatalogueItem({ companyId, itemCode } = {}) {
    var path = this.buildUrl({
   ...
   * @return FetchResult
   */
  getClassificationStatus({ companyId, includeClassificationDetails, filter, top, skip, orderBy } = {}) {
    var path = this.buildUrl({
      url: `/api/v2/companies/${companyId}/classificationrequests/taxcode`,
      parameters: {
   ...
   * @return object
   */
  getPremiumClassification({ companyId, itemCode, systemCode } = {}) {
    var path = this.buildUrl({
   ...
   * 
     * @param int companyId The ID of the company that defined these items
     * @param int requestId The ID of the classification request
   ...
   * @return FetchResult
   */
  getTaxCodeRecommendations({ companyId, requestId, filter, top, skip, orderBy } = {}) {
    var path = this.buildUrl({
   ...
   * @return FetchResult
   */
  listImportRestrictions({ companyId, itemCode, countryOfImport, top, skip, orderBy } = {}) {
    var path = this.buildUrl({
   ...
   * 
     * @param int companyId The ID of the company that owns this item.
     * @param object[] model The items you want to create or update.
   * @return object
   */
  syncItemCatalogue({ companyId, model } = {}) {
    var path = this.buildUrl({
   ...
   *
   * 
     * @param object model The subscription type object you wish to create.
   * @return object
   */
  createServiceTypes({ model } = {}) {
    var path = this.buildUrl({
   ...
   * @return string
   */
  resetPassword({ userId, isUndoMigrateRequest, model } = {}) {
    var path = this.buildUrl({
   ...
   * 
     ...
     * @param object model The subscription type object to update.
   * @return object
   */
  updateServiceType({ id, model } = {}) {
    var path = this.buildUrl({
      url: `/api/v2/servicetypes/${id}`,
      parameters: {}
    });
   ...
   * @return object[]
   */
  deleteUserDefinedField({ companyId, id } = {}) {
    var path = this.buildUrl({
   ...
   * @return FetchResult
   */
  listUserDefinedFieldsByCompanyId({ companyId, udfType, allowDefaults } = {}) {
    var path = this.buildUrl({
   ...
   * @return object
   */
  updateUserDefinedField({ companyId, id, model } = {}) {
    var path = this.buildUrl({
      url: `/api/v2/companies/${companyId}/userdefinedfields`,
      parameters: {
        id: id
   ...
   * @return 
   */
  deregisterShipment({ companyCode, transactionCode, documentType, api_version= "", x_avalara_version= "" } = {}) {
    var path = this.buildUrl({
   ...
   * @return 
   */
  registerShipment({ companyCode, transactionCode, documentType, api_version= "", x_avalara_version= "" } = {}) {
    var path = this.buildUrl({
      url: `/api/v2/companies/${companyCode}/transactions/${transactionCode}/shipment/registration`,
      parameters: {
        documentType: documentType,
