...
export default class AvaTaxClient {
  ...
  restCall({ url, verb, payload }) {
    return fetch(url, {
      ...
      var contentType = res.headers._headers['content-type'][0] ? res.headers._headers['content-type'][0] : '';
   ...
   * @return FetchResult
   */
  getAccrualFillings({ companyId, filingReturnId } = {}) {
    var path = this.buildUrl({
