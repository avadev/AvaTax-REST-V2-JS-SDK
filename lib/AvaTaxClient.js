...
import { createBasicAuthHeader } from './utils/basic_auth.js';
...
/**
   ...
   */
 constructor({ appName, appVersion, machineName, environment,user=null}) {
    this.baseUrl = 'https://rest.avatax.com';
    if(user){

      if(user.maxRetryattempts<=0)
        {
          user.maxRetryattempts=0;
        }
      if(user.TimeoutInMinutes<=0)
        {
          user.TimeoutInMinutes=2;
        }
       this.user=user;
      }
    else{
      this.user=new UserConfiguration(0,2);
    }   
...
let arr=[];
for(let i=1;i<=this.user.maxRetryattempts;i++)
arr.push(2*i*1000);  
    return polly().
    waitAndRetry(arr)  
    .executeForPromise(()=>{  
    return withTimeout(this.user.TimeoutInMinutes*60*1000, fetch(url, {
    method: verb,
    headers: {
