...
export default class AvaTaxClient {
  /**
     ...
     */
  constructor({ appName, appVersion, machineName, environment, userConfig = null }) {
    this.baseUrl = 'https://rest.avatax.com';
    if (userConfig) {

      if (userConfig.maxRetryattempts <= 0) {
        userConfig.maxRetryattempts = 0;
      }
      if (userConfig.TimeoutInMinutes <= 0) {
        userConfig.TimeoutInMinutes = 2;
      }
      this.userConfig = userConfig;
    }
    else {
      this.userConfig = new UserConfiguration(0, 2);
    }
    ...
    this.clientId = appName + '; ' + appVersion + '; JavascriptSdk; 21.3.2; ' + machineName;
  }
  ...
  restCall({ url, verb, payload }) {
    let arr = [];
    for (let i = 1; i <= this.userConfig.maxRetryattempts; i++) {
      arr.push(2 * i * 1000);
    }
    return polly().
      waitAndRetry(arr)
      .executeForPromise(() => {
        return withTimeout(this.userConfig.TimeoutInMinutes * 60 * 1000, fetch(url, {
          method: verb,
          headers: {
          ...
          else if (err.code && err.code == 'InternalServerError') {
            return Promise.reject(err)
          }
          else {
            return err;
