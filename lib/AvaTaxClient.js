...
export default class AvaTaxClient {
   ...
   * @return string
   */
  createAdvancedRuleScript(
    { accountId, scriptType, crashBehavior, file } = {}
  ) {
    var path = this.buildUrl({
   ...
   */
  listFilingRequests(
    { companyId, filingCalendarId, filter, top, skip, orderBy } = {}
  ) {
    var path = this.buildUrl({
...
This gets the basic information from the filings and doesn't include anything extra.
   ...
   * @return object[]
   */
  commentDetailsDelete({ companyId, id, commentDetailsId } = {}) {
    var path = this.buildUrl({
   ...
   * @return object[]
   */
  financedetailsdelete({ companyId, id, financeDetailsId } = {}) {
    var path = this.buildUrl({
   ...
   */
  adjustTransaction(
    { companyCode, transactionCode, documentType, model } = {}
  ) {
    var path = this.buildUrl({
