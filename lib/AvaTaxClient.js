...
export default class AvaTaxClient {
  ...
  requestExemptionSetup({ companyId, model } = {}) {
    var path = this.buildUrl({
      url: `/api/v2/companies/${companyId}/certificates/setup`,
      parameters: {}
    });
   ...
   * @return FetchResult
   */
  listCrossBorderCodes({ country, hsCode } = {}) {
    var path = this.buildUrl({
...
This gets the basic information from the filings and doesn't include anything extra.
   ...
   * @return object
   */
  downloadTaxRatesByZipCode({ date } = {}) {
    var path = this.buildUrl({
   ...
   */
  refundTransaction(
    { companyCode, transactionCode, include, documentType, model } = {}
  ) {
    var path = this.buildUrl({
