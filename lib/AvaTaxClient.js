...
export default class AvaTaxClient {
   ...
   */
  constructor({ appName, appVersion, machineName, environment, headers }) {
   ...
   */
   ...
   createRestOptions(verb, payload ) {
      const headers = this.headers;
      if (/^(get|head)$/i.test(verb)){
        if(!!payload){
          throw new Error('Payload/body cannot be defined for GET/HEAD requests');
        }
        return { headers, method:verb }
      }
      return { headers, 
        body: JSON.stringify(payload), method: verb  }
   }
  ...
  restCall({ url, verb, payload }) {
    const options = this.createRestOptions(verb, payload);

    return fetch(url, options ).then(res => {
      const contentHeaders = res.headers && res.headers._headers && res.headers._headers['content-type'];
	    const contentType = contentHeaders && contentHeaders[0];
